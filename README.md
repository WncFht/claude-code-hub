...
- Linux / macOS:
  ...
  ```bash
  ./deploy.sh --probe-only -d /path/to/runtime
  ./deploy.sh --restart -d /path/to/runtime
  ```

- Windows:

  ```powershell
  .\deploy.ps1 -ProbeOnly -DeployDir C:\ProgramData\claude-code-hub
  .\deploy.ps1 -Restart -DeployDir C:\ProgramData\claude-code-hub
  ```

- 若 runtime 使用了额外 compose 文件，可显式传入 `--compose-file` / `-ComposeFile`；若 `.env` 不在默认位置，也可以传入 `--env-file` / `-EnvFile`。脚本会打印 `docker compose ps`、容器状态、端口映射、宿主机健康检查、容器内健康检查，并在失败时自动附带最近日志。
...
当你想保留现有 PostgreSQL / Redis / `.env`，但仅用本地最新代码重建 `app` 容器时，可以使用仓库内置的 overlay 文件。

1. 先在源码目录完成生产构建：
   ```bash
   bun run build
   ```
2. 准备本地 build context：
   ```bash
   mkdir -p local-build-context
   rsync -a --delete .next/ local-build-context/.next/
   rsync -a --delete public/ local-build-context/public/
   rsync -a --delete messages/ local-build-context/messages/
   ```
3. 仅重建 `app`：
   ```bash
   docker compose -f docker-compose.yaml -f docker-compose.local-overlay.yaml up -d --build app
   ```
4. Linux 如需 `host-gateway`，再叠加 Linux override：
   ```bash
   docker compose \
     -f docker-compose.yaml \
     -f docker-compose.local-overlay.yaml \
     -f docker-compose.local-overlay.linux.yaml \
     up -d --build app
   ```
5. 如需把“构建 + 同步 build context + runtime compose 重建 + 健康检查”串成一次可重复修复，直接使用仓库脚本：
   ```bash
   scripts/repair-local-overlay.sh
   ```
   `archbox` 这类 runtime compose 不在仓库目录里的部署，可以显式指定 runtime 位置：
   ```bash
   scripts/repair-local-overlay.sh \
   ...
   ```
   若只是重试 Docker 重建而不重新 `bun run build`，可加 `--skip-build`。

说明：

- `docker-compose.local-overlay.yaml` 只覆盖 `app` 镜像来源，默认使用相对路径，因此在 macOS / Linux / Windows 换目录时都不依赖绝对路径。
- 如需自定义路径或镜像名，可设置 `LOCAL_OVERLAY_CONTEXT`、`LOCAL_OVERLAY_DOCKERFILE`、`LOCAL_OVERLAY_IMAGE`。
- `scripts/repair-local-overlay.sh` 不会修改 provider 配置；它只负责 overlay 构建产物、Docker compose 重建与健康检查。
