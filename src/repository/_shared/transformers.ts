...
export function toProvider(dbProvider: any): Provider {
  return {
    ...
    discoveredModels: dbProvider?.discoveredModels ?? null,
    modelDiscoveryStatus: dbProvider?.modelDiscoveryStatus ?? null,
    lastModelSyncAt: dbProvider?.lastModelSyncAt ? new Date(dbProvider.lastModelSyncAt) : null,
    lastModelSyncError: dbProvider?.lastModelSyncError ?? null,
