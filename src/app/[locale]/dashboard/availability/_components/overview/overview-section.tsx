...
import type { ClientAbortCounters } from "@/lib/availability";
...
interface OverviewSectionProps {
  ...
  avgLatency: number;
  errorRate: number;
  clientAbortCounts: ClientAbortCounters | null;
  activeProbes: number;
  totalProbes: number;
  loading?: boolean;
  refreshing?: boolean;
...
export function OverviewSection({
  ...
  return (
    <div className="space-y-4">
      ...
      {clientAbortCounts && clientAbortCounts.total > 0 ? (
        <div
          className={cn(
            "rounded-2xl border border-border/50 dark:border-white/[0.08] bg-card/60 dark:bg-[rgba(20,20,23,0.5)] p-4 backdrop-blur-lg",
            refreshing && "opacity-70 transition-opacity"
          )}
        >
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">{t("clientAbort.description")}</p>
            </div>
            <div className="flex flex-wrap gap-2 text-xs">
              <span className="rounded-full border border-border/60 px-2.5 py-1">
                {t("clientAbort.total", { count: clientAbortCounts.total })}
              </span>
              <span className="rounded-full border border-border/60 px-2.5 py-1">
                {t("clientAbort.sessionContinued", {
                  count: clientAbortCounts.sessionContinued,
                })}
              </span>
              <span className="rounded-full border border-border/60 px-2.5 py-1">
                {t("clientAbort.afterStreamStart", {
                  count: clientAbortCounts.afterStreamStart,
                })}
              </span>
              <span className="rounded-full border border-border/60 px-2.5 py-1">
                {t("clientAbort.beforeStreamStart", {
                  count: clientAbortCounts.beforeStreamStart,
                })}
              </span>
              {clientAbortCounts.longRunning > 0 ? (
                <span className="rounded-full border border-amber-200 bg-amber-50 px-2.5 py-1 text-amber-700 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-300">
                  {t("clientAbort.longRunning", { count: clientAbortCounts.longRunning })}
                </span>
              ) : null}
            </div>
          </div>
